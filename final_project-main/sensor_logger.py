...
# data_file = 'data/Mixed_Activity/Accelerometer.csv'
data_file = 'data/Mixed_Activity/Accelerometer.csv'
...
def predict(window):
	...
	pred_labels, pred_features = extract_features(window)
	...
	# print(window)
	# print(pred_features)
	predicted_label = classifier.predict([pred_features])
...
while(index < len(data)-window):
	save_len = len(data[index:index+window , 0])
	x_data = x_data + data[index:index+window , 0].tolist()
	y_data = y_data + data[index:index+window , 1].tolist()
	z_data = z_data + data[index:index+window , 2].tolist()
	
	predicted_label = predict(data[index:index+window , :])
	l = predicted_label[0]
	res = np.full(save_len , l)
	l_data = l_data + res.tolist()
	index+=window
	print("l_data = " , index)

print("label array: done")

colors = ['r', 'g', 'b', 'y']
# leg = ["walking", "running", "sitting","plank"]
label_colors = [colors[int(label)] for label in l_data]

plt.figure(figsize=(10, 6))
window_size=100

time_x = my_data[0:len(x_data) , 1]
...
assert(len(time_x) == len(x_data))
for i in range(0, len(x_data), window_size):
	if label_colors[i] == 'r' and rc == 0:
		plt.plot(time_x[i:i+window_size] , x_data[i:i+window_size], color=label_colors[i], label = "walking")
		rc = 1
	if label_colors[i] == 'g' and gc == 0:
		plt.plot(time_x[i:i+window_size] , x_data[i:i+window_size], color=label_colors[i], label = "running")
		gc = 1
	if label_colors[i] == 'b' and bc == 0:
		plt.plot(time_x[i:i+window_size] , x_data[i:i+window_size], color=label_colors[i], label = "sitting")
		bc = 1
	if label_colors[i] == 'y' and yc == 0:
		plt.plot(time_x[i:i+window_size] , x_data[i:i+window_size], color=label_colors[i], label = "planking")
		yc = 1
	
	else:
		plt.plot(time_x[i:i+window_size] , x_data[i:i+window_size], color=label_colors[i])
	
	plt.plot(time_x[i:i+window_size] , y_data[i:i+window_size], color=label_colors[i])
	plt.plot(time_x[i:i+window_size] , z_data[i:i+window_size], color=label_colors[i])

plt.ylabel("Accelerometer")
plt.xlabel("Time")
plt.legend()
#plt.legend(colors, ['walking', 'running', 'sitting', 'planking'] )
plt.show()
